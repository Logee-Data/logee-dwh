...
SELECT
  ...
  IF(IF(REPLACE(JSON_EXTRACT(data, '$.simExpiryDate'), '"', '') = '', NULL, REPLACE(JSON_EXTRACT(data, '$.simExpiryDate'), '"', '')) NOT LIKE '+%', DATE(IF(REPLACE(JSON_EXTRACT(data, '$.simExpiryDate'), '"', '') = '', NULL, REPLACE(JSON_EXTRACT(data, '$.simExpiryDate'), '"', ''))), NULL) AS sim_expiry_date,
  IF(IF(REPLACE(JSON_EXTRACT(data, '$.simValidityPeriod'), '"', '') = '', NULL, REPLACE(JSON_EXTRACT(data, '$.simValidityPeriod'), '"', '')) NOT LIKE '+%', DATE(IF(REPLACE(JSON_EXTRACT(data, '$.simValidityPeriod'), '"', '') = '', NULL, REPLACE(JSON_EXTRACT(data, '$.simValidityPeriod'), '"', ''))), NULL) AS sim_validity_period,
  ...
  CAST(IF(REPLACE(JSON_EXTRACT(data, '$.status'), '"', '') = '', NULL, REPLACE(JSON_EXTRACT(data, '$.status'), '"', '')) AS BOOL) AS status,
