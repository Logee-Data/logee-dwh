...
WITH base as(
  ...
  FROM `logee-data-prod.L1_visibility_playground.lgd_companies`
)
...
,check AS (
 ...
 FROM base
  ...
  SELECT
    ...
    STRUCT(
      'catalogue_product_url' AS column,
      IF(settings.catalogue_product_url IS NULL, 'Column can not be NULL', 'Column can not be an empty string') AS quality_notes
    ) AS quality_check
  FROM base
  WHERE settings.catalogue_product_url IS NULL or settings.catalogue_product_url = ''
