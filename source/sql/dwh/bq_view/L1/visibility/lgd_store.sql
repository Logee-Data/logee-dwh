...
ts AS published_timestamp,
  ARRAY_AGG(
    ...
    REPLACE(JSON_EXTRACT_SCALAR(listAddress,'$.lat'), '"', '') AS lat,
    REPLACE(JSON_EXTRACT_SCALAR(listAddress,'$.long'), '"', '') AS long,
...
,ownerAddress as (  
  ...
  struct(
       ...
       IF(REPLACE(JSON_EXTRACT(data, '$.storeOwner.ownerAddress.lat'), '"', '') = "", NULL, REPLACE(JSON_EXTRACT(data, '$.storeOwner.ownerAddress.lat'), '"', '')) AS lat,
...
B.list_Address,
  ...
  JSON_EXTRACT_SCALAR(A.data, '$.mainAddressId') AS main_address_id,
    ...
    REPLACE(JSON_EXTRACT_SCALAR(A.data, '$.mainAddress.lat'), '"', '') AS lat,
    REPLACE(JSON_EXTRACT_SCALAR(A.data, '$.mainAddress.long'), '"', '') AS long,
