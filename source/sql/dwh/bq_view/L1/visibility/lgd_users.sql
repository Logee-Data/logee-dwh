...
,metadata AS (
  SELECT
    ...
    ts AS published_timestamp,
    STRUCT (
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.metadata.firstOrderId'), '"', '') AS first_order_id
    ) AS metadata
  FROM base
)
-- END METADATA
...
,main_address AS (
  SELECT
    ...
    ts AS published_timestamp,
    STRUCT(
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.storeName') AS store_name,
      IF(REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.recipientName'), '"', '') = "", NULL, REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.recipientName'), '"', ''))  AS recipient_name,
      CAST(JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.phoneNumber') AS INT64) AS phone_number,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.province') AS province,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.provinceId	') AS province_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.city') AS city,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.cityId') AS city_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.district') AS district,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.districtId') AS district_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.subDistrict') AS sub_district,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.subDistrictId') AS sub_district_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.zipCode') AS zip_code,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.zipCodeId') AS zipcode_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.address') AS address,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.mainAddress') AS main_address,
      IF(JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.externalId') = "", NULL, JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.externalId')) AS external_id,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.lat') AS lat,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.long') AS long,
      CAST(JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.isFulfillmentProcess') AS BOOL) AS is_fulfillment_process,
      JSON_EXTRACT_SCALAR(data, '$.usermetadata.mainAddress.addressId') AS address_id,
...
,store_owner AS (
  SELECT
    ...
    ts AS published_timestamp,
    STRUCT (
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerName'), '"', '') AS owner_name,
      CAST(REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerPhoneNumber'), '"', '') AS INT64) AS owner_phone_number,
      STRUCT (
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.province'), '"', '') AS province,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.provinceId'), '"', '') AS province_id,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.city'), '"', '') AS city,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.cityId'), '"', '') AS city_id,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.district'), '"', '') AS district,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.districtId'), '"', '') AS district_id,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.subDistrict'), '"', '') AS sub_district,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.subDistrictId'), '"', '') AS sub_district_id,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.zipCode'), '"', '') AS zip_code,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.zipCodeId'), '"', '') AS zip_code_id,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.address'), '"', '') AS address,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.lat'), '"', '') AS lat,
        REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.storeOwner.ownerAddress.long'), '"', '') AS long
      ) AS owner_address
    ) AS store_owner
  FROM
    base
...
,integrated_account AS (
  SELECT
    ...
    STRUCT(
      STRUCT (
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.userName'), '"', '') AS user_name,
      CAST(REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.phoneNo'), '"', '') AS INT64) AS phone_number,
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.signature'), '"', '') AS signature,
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.idTmoney'), '"', '') AS id_Tmoney,
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.idFusion'), '"', '') AS id_Fusion,
      REPLACE(JSON_EXTRACT_SCALAR(data, '$.usermetadata.integratedAccount.tmoney.token'), '"', '') AS token,
...
,usermetadata AS (
  SELECT
    ...
    ts AS published_timestamp,
    STRUCT(
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.userType') AS user_type,
      F.store_owner,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.salesId') AS sales_id,
      C.list_address,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.storeId') AS store_id,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.companyId') AS company_id,
      IF(JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.externalId') = "", NULL, JSON_EXTRACT(A.data, '$.usermetadata.externalId')) AS external_id,
      ...
      D.main_address,
      IF(JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.assignedTaskId') = "", NULL, JSON_EXTRACT(A.data, '$.usermetadata.assignedTaskId'))  AS assigned_task_id,
      JSON_EXTRACT_ARRAY(A.data, '$.usermetadata.apps') AS apps,
      B.store_images,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.userId') AS user_id,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.storeCode') AS store_code,
      G.integrated_account,
      E.metadata,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.approvedAt') AS approved_at,
      JSON_EXTRACT_SCALAR(A.data, '$.usermetadata.salesEmployeeStatus') AS sales_employee_status,
...
SELECT
  ...
  REPLACE(JSON_EXTRACT(A.data, '$.username'), '"', '') AS username,
  ...
  REPLACE(JSON_EXTRACT(A.data, '$.phoneNumber'), '"', '') AS phoneNumber,
  ...
  D.usermetadata AS usermetadata,
  CAST(REPLACE(JSON_EXTRACT(A.data, '$.isActive'), '"', '') AS BOOL) AS is_active,
